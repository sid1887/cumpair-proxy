...
# default build args (override at build or set env in Render)
ARG PROXY_USER=user
ARG PROXY_PASS=pass

# Create log dir and tinyproxy log
RUN mkdir -p /var/log/tinyproxy && chown -R nobody:nogroup /var/log/tinyproxy

# Create basic tinyproxy config listening on 8080 with BasicAuth
